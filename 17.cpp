...
class Trie
    {
        public:
            ...
            std::vector<int>  find(const std::string& text, std::vector<int> &result) const;
        ...
        private:
            ...
            int getSuffixLink(int state) const;
            int getUpLink(int state) const;
            int getDelta(int state, int stateByChar) const;
    };
...
std::vector<int> Trie::find(const std::string &text, std::vector<int> &result) const
    {
        int position = 0;
...
int Trie::getSuffixLink(int state) const
    {
        int stateSuffixLink = 0;
        if (nodes[state].SuffixLink == -1)
        {
            ...
            {
                stateSuffixLink = getDelta(getSuffixLink(par), sbc);
            }
        }
        return stateSuffixLink;
    }

int Trie::getDelta(int state, int stateByChar) const
    {
        int stateByCharDeltaFunction = 0;
        if (nodes[state].DeltaFunction[stateByChar] == -1)
            if (nodes[state].NextStates[stateByChar] != -1)
            {
                stateByCharDeltaFunction = nodes[state].NextStates[stateByChar];
            }
        else
            {
                stateByCharDeltaFunction = (state == 0) ? 0 : getDelta(getSuffixLink(state), stateByChar);
            }

        return stateByCharDeltaFunction;
    }

int Trie::getUpLink(int state) const
    {
        int stateUpSuffixsLink = 0;
        if (nodes[state].UpSuffixsLink == -1)
        {
            int currentState = getSuffixLink(state);
            if (nodes[currentState].IsTerminal)
            {
                stateUpSuffixsLink = currentState;
            }
            else
            {
                stateUpSuffixsLink = currentState == 0 ? 0 : getUpLink(currentState);
            }
        }

        return  stateUpSuffixsLink;
    }
